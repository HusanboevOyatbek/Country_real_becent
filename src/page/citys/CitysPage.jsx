...
const CitysPage = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (isEditing && currentRecord) {
        await axios.put(`https://v1.turbotravel.uz/api/cities/${currentRecord.id}`, values, {
          headers: {
        ...
        await axios.post(`https://v1.turbotravel.uz/api/cities`, values, {
          headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${token}`
          }
  ...
  return (<>
    ...
    <Table columns={columns} loading={loading} dataSource={cities} />
  </>)
};
